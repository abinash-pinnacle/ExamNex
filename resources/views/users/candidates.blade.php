...
<div class="lg:h-[calc(100%_-_4rem)] lg:flex lg:flex-col lg:min-h-0">
    <div class="shrink-0 mb-4">
        <h1 class="text-2xl font-bold">Candidates <span class="text-slate-400 font-normal text-lg">· Students</span></h1>
        <p class="text-slate-500 text-sm mt-1">Students who take exams. (Staff accounts are under <a href="{{ route('users.index') }}" class="text-brand hover:underline">Users</a>.)</p>
    </div>

    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:flex-1 lg:min-h-0">
        {{-- Add candidate (stays put) --}}
        ...
        {{-- List (fixed area: search stays, table scrolls, pagination pinned) --}}
        <div class="lg:col-span-2 lg:flex lg:flex-col lg:min-h-0">
            <form method="GET" class="flex flex-wrap gap-2 mb-3 text-sm shrink-0">
                ...
                <select name="batch" class="rounded-lg border-slate-300 border px-2 py-2">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden lg:flex-1 lg:min-h-0 lg:flex lg:flex-col">
                <div class="overflow-auto lg:flex-1 lg:min-h-0">
                    <table class="w-full text-sm min-w-[640px]">
                        <thead class="bg-slate-50 text-slate-500 text-left sticky top-0 z-10">
                            <tr><th class="px-4 py-2">Candidate</th><th class="px-4 py-2">ID / Batch</th><th class="px-4 py-2">Assigned</th><th class="px-4 py-2">Attempts</th><th class="px-4 py-2">Active</th><th class="px-4 py-2"></th></tr>
                        </thead>
            ...
            @if ($candidates->hasPages())
                <div class="mt-3 shrink-0">{{ $candidates->onEachSide(1)->links() }}</div>
            @else
                <div class="mt-3 shrink-0 text-xs text-slate-400">{{ $candidates->total() }} candidate(s)</div>
            @endif
        </div>
    </div>
</div>
